class TConsole_IContainter {
public:
    virtual void Call(IArguments *) = 0;
};

template<class F>
class TConsole_Container : public TConsole_IContainter {
};
...
public:
    ...
    TConsole_Container(M func, unsigned char &arg_type) {
        ...
        if (typeid(A1) == typeid(float)) {
            arg_type = 2;
            return;
        };
        if (typeid(A1) == typeid(char *)) {
            arg_type = 4;
            return;
        };
    }
