...
class TConsole
{
	...
	{	
		...
		char parameter[256]; parameter[0] = 0;
		...
		{	
			if (parameter[0] == 0) 
			{
				...
				if (annotations)
				{
					...
					switch (iarg)
					{
						case TC_ARG_NONE : break;
						case TC_ARG_INT : (*this) << " int";break;
						case TC_ARG_FLOAT : (*this) << " float";break;
						case TC_ARG_CHAR : (*this) << " string";break;
						case TC_ARG_COLOR : (*this) << " color";break;
						case TC_ARG_BOOL : (*this) << " bool";break;
	...
	template<class T> T& RegisterVar( T def, const char* name, void (*func)(void) = 0 )
	{
	...
	template< class F > void RegisterFunction( F f, const char* name, const char* annotations = 0 )
	{
    ...
    template< class T > TConsole& operator << ( T in )
    {
    	stringstream ss;
    	ss << in;
    	string st = ss.str();
